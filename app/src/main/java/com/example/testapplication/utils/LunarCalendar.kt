...
object LunarCalendar {
    ...
    private val baseDate = Calendar.getInstance(TimeZone.getTimeZone("UTC")).apply {
        set(1900, 0, 31, 0, 0, 0)
        set(Calendar.MILLISECOND, 0)
    }

    fun solarToLunar(date: Date): Pair<Int, Int> {
        val offset = ((date.time - baseDate.timeInMillis) / 86400000).toInt() - 1
        var days = offset
        var iYear = 1900

        while (iYear < 2100 && days > yearDays(iYear)) {
            days -= yearDays(iYear)
            iYear++
        }

        var iMonth = 1
        val leap = leapMonth(iYear)
        var isLeap = false
        var i = 1

        while (i <= 12 && days > 0) {
            val monthDays = if (leap > 0 && i == leap + 1 && !isLeap) {
                isLeap = true
                leapDays(iYear)
            } else {
        ...
        if (days < 0) {
            days += monthDays(iYear, i)
            i--
        }

        iMonth = i
        val day = days + 1
        return Pair(iMonth, day)
    }
